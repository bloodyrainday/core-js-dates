...
function getNextFriday(date) {
  const dateObj = new Date(date);
  let weekday = dateObj.toLocaleDateString('en', { weekday: 'long' });
  if (weekday === 'Friday') {
  ...
  while (weekday !== 'Friday') {
    ...
    weekday = dateObj.toLocaleDateString('en', { weekday: 'long' });
  }
  return dateObj;
}
...
function getCountDaysInMonth(month, year) {
  const days = new Date(year, month - 1 + 1, 0);
  return days.getDate();
}
...
function isDateInPeriod(date, period) {
  ...
  return dateObj <= end && dateObj >= start;
}
 ...
 * 1, 2024 => 8
 */
function getCountWeekendsInMonth(m, year) {
  const month = m - 1;
...
function getNextFridayThe13th(date) {
  const day13 = new Date(date.getFullYear(), date.getMonth(), 13);
  let weekday = day13.toLocaleDateString('en-US', { weekday: 'long' });
  while (weekday !== 'Friday') {
    day13.setMonth(day13.getMonth() + 1);
    weekday = day13.toLocaleDateString('en-US', { weekday: 'long' });
  }
  return day13;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const startPeriodArray = period.start.split('-');
  const endPeriodArray = period.end.split('-');
  const startPeriod = new Date(
    Date.UTC(startPeriodArray[2], startPeriodArray[1] - 1, startPeriodArray[0])
  );
  const endPeriod = new Date(
    Date.UTC(endPeriodArray[2], endPeriodArray[1] - 1, endPeriodArray[0])
  );
  const schedule = [];
  const workDay = startPeriod;

  while (workDay <= endPeriod) {
    if (countWorkDays > 1) {
      for (let i = 2; i <= countWorkDays; i += 1) {
        if (workDay > endPeriod) {
          break;
        }
        schedule.push(workDay.toLocaleDateString('ru').split('.').join('-'));
    ...
    schedule.push(workDay.toLocaleDateString('ru').split('.').join('-'));
    workDay.setDate(workDay.getDate() + countOffDays + 1);
  }
  return schedule;
}
...
function isLeapYear(date) {
  ...
  if (Math.abs(leapYear - currentYear) % 4 === 0) {
